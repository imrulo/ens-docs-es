---
description: Aprende sobre cómo CCIP Read habilita Resolvers ENS Fuera de Cadena, cómo funciona una puerta de enlace, supuestos de confianza, y más.
---
...
# Resolvers Fuera de Cadena / L2

Aunque la resolución de nombres ENS siempre comienza desde Ethereum Mainnet, es posible almacenar casi todos los datos asociados con un nombre y sus subdominios en otro lugar. Aprovechando [EIP-3668](https://eips.ethereum.org/EIPS/eip-3668) (CCIP Read) en un [Resolver](/es/resolvers/quickstart), los desarrolladores pueden efectivamente diferir la resolución a una API L2 o fuera de cadena.
...
CCIP Read (Protocolo de Interoperabilidad Cross Chain) es una especificación que define un error estándar que los contratos inteligentes pueden lanzar si quieren activar una solicitud HTTP fuera de cadena.
...
5. La puerta de enlace procesa la solicitud y devuelve datos al cliente. Aquí es donde se obtienen los datos de L2 o una base de datos fuera de cadena.
...
Aunque esto pueda sonar complejo, todo sucede bajo el capó y es completamente transparente para los desarrolladores de aplicaciones.

## Ejemplo de Subnombre Fuera de Cadena

Un ejemplo de nombres ENS fuera de cadena impulsados por CCIP Read se puede encontrar en [offchain.ens.gregskril.com](https://offchain.ens.gregskril.com/).
...
La Puerta de Enlace devuelve la información relevante de una base de datos fuera de cadena, firmada por una clave privada de confianza que el contrato inteligente puede verificar. Esto previene que una Puerta de Enlace comprometida devuelva información falsa.

<Repository
  src="gskril/ens-offchain-registrar"
  description="Subnombres ENS Fuera de Cadena"
/>

## Resolvers Fuera de Cadena vs L2

Desde la perspectiva del contrato Resolver L1, el proceso de resolver un nombre L2 es exactamente el mismo que resolver un nombre fuera de cadena. Las diferencias vienen de la implementación de la Puerta de Enlace y la función de callback del Resolver.

Para nombres que se almacenan fuera de cadena como el ejemplo anterior, la Puerta de Enlace leería de una base de datos web2 normal y la función de callback del Resolver simplemente verificaría la firma del operador de la Puerta de Enlace.
...
## Escribir una Puerta de Enlace CCIP Read

Una puerta de enlace es un endpoint de API fuera de cadena que implementa la [Interfaz de Puerta de Enlace](https://eips.ethereum.org/EIPS/eip-3668#gateway-interface) especificada en EIP-3668. Es responsable de decodificar los datos de llamada de un error `OffchainLookup` y devolver una respuesta relevante.
...
<EmbedLink
  title="Implementación de Puerta de Enlace Básica"
  href="https://github.com/ensdomains/docs/blob/master/functions/api/example/basic-gateway.ts"
/>

### Supuestos de Confianza

Como se explica en [Resolvers Fuera de Cadena vs L2](#offchain-vs-l2-resolvers), los supuestos de confianza dependen del desarrollador implementador y pueden variar desde completamente confiados hasta completamente sin confianza.

El peor escenario de una implementación confiada es que un actor malicioso gane control de la puerta de enlace y pueda devolver información falsa.
