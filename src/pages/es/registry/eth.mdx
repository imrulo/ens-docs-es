...
Esta tarifa se paga en ETH y se establece para prevenir el spam del registrador.
...
Esto previene el acaparamiento de nombres y asegura que el nombre vaya al mejor postor de manera justa.
...
Lee más sobre nuestro gasto en [Artículo III de la Constitución](/dao/constitution#iii-income-funds-ens-and-other-public-goods).
...
Registrar un nombre es un proceso sin confianza que tiene lugar en cadena (más sobre esto abajo). Algunos frontends de código abierto para registrar nombres son la [Aplicación ENS Manager](https://app.ens.domains/), [ENS Fairy](https://ensfairy.xyz/), Rainbow Wallet.
...
El ETHRegistrarController, el contrato de nivel más alto a través del cual los usuarios registran nombres, implementa un esquema commit reveal para prevenir el frontrunning de registros.
...
makeCommitment(
    ...
    0 // Los fusibles de NameWrapper que quieres establecer
);
```
...
Una vez que hayas hecho el compromiso en cadena y esperado 60 segundos, puedes registrar tu nombre.
...
Debido a que el precio de alquiler se paga en ETH pero se denomina en USD, se recomienda a los llamadores enviar ligeramente más que el valor devuelto por `rentPrice` para evitar problemas con cambios rápidos de precio. Una prima de 3-5% probablemente será suficiente.
...
Al permitir renovación por cualquier cantidad de tiempo arbitraria, los usuarios pueden asegurar que su nombre no expire.
Según la separación entre registro y controlador, incluso con controlador actualizado tu nombre seguirá siendo tuyo.
...
// Obtener Marca de Tiempo de Compromiso
...
// Obtener Expiración del Nombre (marca de tiempo unix en la que expira el registro)
...
// Obtener Fin del Período de Transferencia (marca de tiempo unix en la que termina el período de transferencia (del registrador legacy))
