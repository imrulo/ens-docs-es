...
:::note
No todos los dominios de primer nivel soportan DNSSEC, y algunos podrían tener implementaciones ENS personalizadas. Por favor, consulta la [Lista de TLD](/es/dns/tlds) para información específica de TLD.
:::

## DNSSEC

DNSSEC (Extensiones de Seguridad del Sistema de Nombres de Dominio) es una capa adicional de seguridad sobre DNS que permite la verificación criptográfica de registros. Establece una cadena de confianza desde la clave raíz (que está firmada por ICANN) hasta cada clave.
...
No hay tarifa de protocolo ENS para importar un nombre DNS, pero requiere una gran cantidad de gas (hasta unos pocos millones) para enviar la prueba en cadena. Continúa leyendo para aprender cómo esto ha sido mitigado.

## Verificación Fuera de Cadena (Sin Gas)

[EP 5.1](/dao/proposals/5.1) introdujo un nuevo DNSSECOracle y DNSRegistrar que hace posible realizar verificación DNSSEC en tiempo de consulta, habilitando el uso verdaderamente gratuito de nombres DNS en ENS. Llamamos a esto "DNSSEC sin gas".

Funciona habilitando [resolución comodín](/ensip/10) a nivel de TLD DNS. Durante el [proceso de resolución de nombres](/es/resolution), si un nombre no existe ya en cadena pero ha sido configurado para uso en ENS, la prueba DNSSEC será obtenida fuera de cadena vía [CCIP Read](https://eips.ethereum.org/EIPS/eip-3668) y luego verificada en cadena con el mismo DNSSECOracle mencionado anteriormente.
...
Si al menos un dominio ha sido importado para este TLD (vía el método en cadena), el propietario será el `DNSRegistrar` o un contrato inteligente controlado por el operador de registro respectivo.

Si un TLD aún no ha sido activado, el `propietario` devolverá `0x0` y podría requerir que un usuario importe un nombre en cadena para activar el TLD. Ver la [lista de TLD soportados](/es/dns/tlds) para más información.

### Programar Pruebas DNSSEC
...
#### Enviar la prueba al DNSRegistrar
