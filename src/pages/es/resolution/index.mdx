...
Aquí hay un diagrama de algunos de los contratos involucrados al resolver un nombre.
...
El proceso de resolución involucra múltiples partes. Más notablemente el [Registro](/registry/ens), múltiples Registradores ([Registrador ETH](/registry/eth), [Registrador DNS](/registry/dns), [Registrador Inverso](/registry/reverse), etc)
y el concepto de un [Resolver](/resolvers/quickstart).

## Cómo resolver

Aquí hay una pequeña mirada a lo que sucede bajo el capó de tu biblioteca favorita cuando haces una búsqueda de nombre.

### 1. Encontrar el Resolver

Cada nombre tiene un "resolver". Un resolver es simplemente un contrato que implementa la [especificación del resolver](/resolvers/quickstart) y puede ser consultado para información sobre un nombre.
...
Además, todas las funciones anteriores pueden enviarse a la función `resolve()`, especificada en [ENSIP-10](/ensip/10).
Esto permite no solo funcionalidad multicall, sino también implementación más fácil de EIP-3668, y más.
La mayoría de clientes y muchos resolvers utilizan la resolución wildcard como su forma principal de resolución.
...
Este campo de nombre devuelve el nombre "preferido" para la dirección. Siempre debes seguir una búsqueda inversa con una búsqueda hacia adelante para verificar que el nombre resultante apunte de vuelta a la dirección original. Si la dirección no coincide, muestra la dirección en lugar del nombre invertido.
...
Ten en cuenta que muchas bibliotecas ya tienen funcionalidad para hacer esto. Puedes leer más sobre esto en la sección [Obtener un Nombre Primario](/web/reverse).
