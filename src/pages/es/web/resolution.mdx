---
title: Búsqueda de Direcciones [Aprende cómo resolver direcciones de blockchain desde nombres legibles por humanos con ENS.]
description: El protocolo ENS tiene como objetivo facilitar el uso de Ethereum proporcionando una forma simple de usar nombres legibles por humanos en lugar de direcciones largas legibles por máquinas.
---
...
# Búsqueda de Direcciones [Aprende cómo resolver direcciones de blockchain desde nombres legibles por humanos con ENS.]

El protocolo ENS tiene como objetivo facilitar el uso de Ethereum.
Para esto, proporciona una forma simple de usar nombres legibles por humanos en lugar de direcciones largas legibles por máquinas.
...
Ten en cuenta que todas las cadenas separadas por puntos deben tratarse como nombres ENS potenciales, ya que ENS soporta [muchos TLDs](/dns/tlds). Un error común es tratar solo las cadenas que terminan en `.eth` como nombres ENS.
...
export const Name = () => {
  const { data: ensName } = useEnsAddress({
    ...
    chainId: 1, // La cadena para comenzar la resolución (Ethereum Mainnet o una testnet)
  })
...
Cualquier dirección de blockchain (BTC, LTC, SOL, etc.) puede ser consultada por [SLIP-0044](https://github.com/satoshilabs/slips/blob/master/slip-0044.md) coin type o un valor derivado de un EVM Chain ID (especificado en [ENSIP-11](/ensip/11)). Esto incluye redes Ethereum L2 como OP Mainnet y Base.

Para EVM Chains además de Ethereum Mainnet, siempre usa su [ENSIP-11](/ensip/11) coin type, independientemente de estar incluido en SLIP-0044 (como Ether Classic).
...
export const MyAddresses = () => {
  // SLIP-0044 Coin Types (ver ENSIP-9)
...
### Decodificar Hashes de Direcciones
